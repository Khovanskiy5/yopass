...
3. **Настройка фронтенда:**
   ...
   cd website/
   bun install
   bun dev  # Запускает сервер разработки на http://localhost:3000
   ```
...
# Проверка типов
bun run build  # Включает компиляцию TypeScript
```
...
- [ ] **Линтинг пройден** — `bun run lint` (фронтенд) и `golangci-lint run` (бэкенд)
